...
async def verify_risk_api():
    ...
    async with get_session() as session:
        ...
        item_high = ItemModel(
            ...
            classification_code=None, quantity=Decimal("0.0"),
            family="High Risk Family", type_name="Type A"
        )
        session.add(item_high)
        
        match_high = MatchResultModel(
            id=uuid4(), item_id=item_high.id, price_item_id=None,
            confidence_score=50.0, decision="manual-review",
        ...
        item_low = ItemModel(
            ...
            family="Low Risk Family", type_name="Type B"
        )
        session.add(item_low)
        
        match_low = MatchResultModel(
            id=uuid4(), item_id=item_low.id, price_item_id=None, # No price but high conf
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                found_item = next((i for i in high_risk_items if i["family"] == "High Risk Family"), None)
                if found_item:
                    print(f"   ✅ Found high risk item: {found_item['family']} (Score: {found_item['score']})")
