...
bimcalc web serve --host 0.0.0.0 --port 8000
```
...
# Serve the browser UI (exposes http://localhost:8000)
...
- The web UI is available at `http://localhost:8000` once the `app` service is running; approve actions immediately write mappings + audit rows.
