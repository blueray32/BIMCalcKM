    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            item = ItemModel(
                ...
                type_name=str(
                    row.get("Reference") or row.get("Cable ID") or "Unknown Type"
                ),
...
async def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python scripts/ingest_tritex_schedules.py <file_path_or_directory> ..."
        )
        sys.exit(1)
        ...
        if os.path.isdir(path):
            ...
            for root, _, files in os.walk(path):
                for file in files:
                    if (
                        file.endswith(".xlsx")
                        and ("Schedule" in file or "DB" in file)
