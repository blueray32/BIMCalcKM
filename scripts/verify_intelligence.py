...
from bimcalc.db.models import ItemModel, PriceItemModel, TrainingExampleModel
from bimcalc.review.models import ReviewRecord, ReviewItem, ReviewPrice
...
async def verify_intelligence():
    ...
    async with get_session() as session:
        ...
        await session.execute(
            text(f"DELETE FROM training_examples WHERE org_id = '{org_id}'")
        )
        await session.execute(
            text("DELETE FROM match_results WHERE created_by = 'verify-script'")
        )
    ...
    async with get_session() as session:
        ...
        await approve_review_record(
            session, record, created_by="verify-script", annotation="Correcting class"
        )
        await session.commit()
