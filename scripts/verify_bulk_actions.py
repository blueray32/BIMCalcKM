...
from bimcalc.web.app_enhanced import BulkUpdateRequest, bulk_update_matches
...
async def test_bulk_actions():
    async with get_session() as session:
        ...
        await session.execute(text(f"DELETE FROM match_results WHERE item_id IN (SELECT id FROM items WHERE org_id = '{org_id}')"))
        ...
        item1 = ItemModel(
            ...
            family="Test Wall", type_name="Type A", quantity=10, unit="m2"
        )
        item2 = ItemModel(
            id=uuid.uuid4(), org_id=org_id, project_id=project_id, 
            family="Test Wall", type_name="Type B", quantity=20, unit="m2"
        )
        session.add_all([item1, item2])
        await session.flush()
        
        match1 = MatchResultModel(
            id=uuid.uuid4(), item_id=item1.id, confidence_score=85.0, 
            source="fuzzy_match", decision="manual-review", reason="test", created_by="test"
        )
        match2 = MatchResultModel(
            id=uuid.uuid4(), item_id=item2.id, confidence_score=90.0, 
            source="fuzzy_match", decision="manual-review", reason="test", created_by="test"
        )
        ...
        print("Skipping approve test (requires full price item setup). Testing Reject...")
        ...
        # Execute logic
        stmt = select(MatchResultModel).where(MatchResultModel.id.in_(req_reject.match_result_ids))
        ...
        if updated_match2.decision == "rejected" and updated_match2.reason == "Bulk rejected via test":
