...
async def main():
    ...
    async with get_session() as session:
        ...
        await session.execute(
            delete(ProjectModel).where(ProjectModel.project_id == "settings-test")
        )
        ...
        assert project.settings["auto_approval_threshold"] == 90, (
            "Auto-approval threshold not saved"
        )
        assert project.settings["risk_thresholds"]["high"] == 85, (
            "Risk high threshold not saved"
        )
        assert project.settings["risk_thresholds"]["medium"] == 60, (
            "Risk medium threshold not saved"
        )
        ...
        assert project.settings["default_markup_percentage"] == 0.0, (
            "Updated markup not saved"
        )
        assert project.settings["blended_labor_rate"] == 45.0, (
            "Updated labor rate not saved"
        )
        ...
        assert project.settings["default_markup_percentage"] == 0.0, (
            "Markup was lost during partial update"
        )
        assert project.settings["currency"] == "GBP", (
            "Currency was lost during partial update"
        )
