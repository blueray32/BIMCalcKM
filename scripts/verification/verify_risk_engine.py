...
async def verify_risk_engine():
    ...
    item1 = ItemModel(
        id=uuid4(), org_id=org_id, project_id=project_id,
        classification_code="123", quantity=Decimal("10.0"),
        family="Test", type_name="Test"
    )
    price1 = PriceItemModel(
        id=uuid4(), org_id=org_id, item_code="P1", region="EU",
        unit_price=Decimal("100.00"), currency="EUR",
        source_name="Vendor A", description="Desc", unit="ea", sku="SKU1",
        classification_code="123"
    )
    match1 = MatchResultModel(
        id=uuid4(), item_id=item1.id, price_item_id=price1.id,
        confidence_score=100.0, decision="auto-accepted",
        source="fuzzy", reason="perfect", created_by="system"
    )
    ...
    item2 = ItemModel(
        id=uuid4(), org_id=org_id, project_id=project_id,
        classification_code=None, quantity=Decimal("0.0"), # Missing class & quantity
        family="Test", type_name="Test"
    )
    match2 = MatchResultModel(
        id=uuid4(), item_id=item2.id, price_item_id=None,
        confidence_score=50.0, decision="manual-review", # Low confidence & manual review
        source="fuzzy", reason="weak", created_by="system"
    )
