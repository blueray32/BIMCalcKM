...
async def verify_compliance():
    ...
    rule_class = ComplianceRuleModel(
        id=uuid4(), org_id=org_id, name="Class Required", description="Must have class",
        rule_type="classification_required", severity="high", is_active=True
    )
    
    rule_vendor = ComplianceRuleModel(
        id=uuid4(), org_id=org_id, name="Vendor Whitelist", description="Approved vendors only",
    ...
    item_good = ItemModel(
        ...
        family="Good Item", type_name="Type Y"
    )
    price_good = PriceItemModel(
        id=uuid4(), org_id=org_id, item_code="P2", region="EU",
        unit_price=Decimal("10.0"), currency="EUR",
        source_name="Approved Vendor A", # In whitelist
        description="Desc", unit="ea", sku="SKU2",
        classification_code="123", source_currency="EUR"
    )
