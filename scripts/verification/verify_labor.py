...
async def verify():
    async with AsyncSessionLocal() as session:
        ...
        await session.execute(
            text("DELETE FROM price_items WHERE vendor_id = 'TEST_LABOR'")
        )
        await session.execute(text("DELETE FROM items WHERE org_id = 'test_org'"))
        await session.execute(
            text("DELETE FROM match_results WHERE created_by = 'test_script'")
        )
        await session.execute(
            text("DELETE FROM item_mapping WHERE created_by = 'test_script'")
        )
        ...
        for item in items:
            print(
                f"Item: {item.sku}, Price: {item.unit_price}, Labor Hours: {item.labor_hours}"
            )
        ...
        item_res = await session.execute(
            text(
                "SELECT id, canonical_key, org_id FROM items WHERE org_id = 'test_org'"
            )
        )
        ...
        pi_res = await session.execute(
            text(
                "SELECT id, labor_hours FROM price_items WHERE vendor_id = 'TEST_LABOR'"
            )
        )
        pi_row = pi_res.first()
        ...
        map_res = await session.execute(
            text(
                "SELECT price_item_id, canonical_key FROM item_mapping WHERE org_id = 'test_org'"
            )
        )
        ...
        match_res = await session.execute(
            text(
                "SELECT item_id, decision, price_item_id FROM match_results WHERE created_by = 'test_script'"
            )
        )
