...
async def verify_export_filters():
    ...
    async with get_session() as session:
        ...
        try:
            ...
            response = client.get(
                f"/api/projects/{project_str}/export/csv/items?category=Cat A"
            )
            ...
            response = client.get(
                f"/api/projects/{project_str}/export/csv/prices?vendor=Vendor X"
            )
            assert response.status_code == 200
            content = response.text
            ...
            response = client.get(
                f"/api/projects/{project_str}/export/csv/matches?min_confidence=90"
            )
            assert response.status_code == 200
            content = response.text
