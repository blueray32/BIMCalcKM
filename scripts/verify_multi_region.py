...
from bimcalc.matching.orchestrator import match_item
...
async def verify_multi_region():
    ...
    async with get_session() as session:
        ...
        print("   Creating EU Price: €100.00")
        session.add(
            PriceItemModel(
                org_id=org_id,
                item_code=sku,
        ...
        print("   Creating US Price: $120.00")
        session.add(
            PriceItemModel(
                org_id=org_id,
                item_code=sku,
        ...
        if price_eu and price_eu.currency == "EUR" and price_eu.unit_price == 100.00:
            print(
                f"   ✅ EU Match Success: Found {price_eu.currency} {price_eu.unit_price}"
            )
        else:
            print(
                f"   ❌ EU Match Failed: Got {price_eu.currency if price_eu else 'None'} {price_eu.unit_price if price_eu else ''}"
            )
        ...
        if price_us and price_us.currency == "USD" and price_us.unit_price == 120.00:
            print(
                f"   ✅ US Match Success: Found {price_us.currency} {price_us.unit_price}"
            )
        else:
            print(
                f"   ❌ US Match Failed: Got {price_us.currency if price_us else 'None'} {price_us.unit_price if price_us else ''}"
            )
