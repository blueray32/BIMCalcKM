...
async def verify_compliance():
    ...
    async with get_session() as session:
        ...
        await session.execute(
            delete(ComplianceRuleModel).where(
                ComplianceRuleModel.project_id == project_id
            )
        )
        await session.execute(
            delete(ItemModel).where(ItemModel.project_id == project_id)
        )
        ...
        if pass_count >= 2 and fail_count == 1:
            ...
            print(
                f"❌ Verification Failed. Expected 2 Pass, 1 Fail. Got {pass_count} Pass, {fail_count} Fail."
            )
            for r in results:
