...
async def verify_performance():
    ...
    async with get_session() as session:
        ...
        await session.execute(text("DELETE FROM documents WHERE title LIKE 'PerfTest%'"))
        ...
        if bind.dialect.name == 'sqlite':
            ...
            results = sorted(all_docs, key=lambda d: cosine_dist(d.embedding, query_vec))[:5]
        else:
            # Postgres: Use pgvector operator
            stmt = select(DocumentModel).order_by(DocumentModel.embedding.cosine_distance(query_vec)).limit(5)
