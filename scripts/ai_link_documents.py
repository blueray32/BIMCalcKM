...
async def extract_item_ids_from_text(text: str, model: str = "gpt-4o-mini") -> list[str]:
...
"""
    ...
    try:
        response = await asyncio.to_thread(
            ...
            model=model,
            messages=[
                {"role": "system", "content": "You are an expert at extracting equipment IDs from technical documents."},
...
async def link_document_to_items(
    ...
    for item_id in item_ids:
        ...
        stmt = select(ItemModel).where(
            ItemModel.type_name.ilike(f"%{item_id}%")
        )
...
async def main():
    ...
    parser.add_argument("--test", action="store_true", help="Test mode: process only first 5 documents")
    ...
    async with get_session() as session:
        ...
        for doc in documents:
            ...
            item_ids = await extract_item_ids_from_text(doc.content, model="gpt-4o-mini")
