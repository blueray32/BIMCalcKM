...
class ComplianceRiskScorer:
    ...
    async def calculate_risk(
        ...
        if doc_count == 0:
            ...
            score += 20
            factors['doc_coverage'] = {'score': 20, 'status': f'{doc_count} documents (limited)'}
        else:
            factors['doc_coverage'] = {'score': 0, 'status': f'{doc_count} documents (good)'}
        ...
        if not class_code:
            ...
            score += 15
            factors['classification'] = {'score': 15, 'status': f'Complex ({class_code})'}
        elif class_code in self.SIMPLE_CLASSIFICATIONS:
            score += 5
            factors['classification'] = {'score': 5, 'status': f'Simple ({class_code})'}
        else:
            score += 10
            factors['classification'] = {'score': 10, 'status': f'Standard ({class_code})'}
