...
class ChecklistPDFExporter:
    ...
    def export(self, checklist: QAChecklistModel, item: ItemModel) -> bytes:
        ...
        # Progress bar (visual)
        elements.append(
            HRFlowable(width="100%", thickness=2, color=colors.HexColor("#e2e8f0"))
        )
        ...
        for category, cat_items in categories.items():
            ...
            # Create table
            table = Table(
                table_data, colWidths=[0.5 * inch, 4 * inch, 1 * inch, 0.8 * inch]
            )
            table.setStyle(
                TableStyle(
                    [
        ...
        if checklist.source_documents.get("docs"):
            elements.append(Spacer(1, 20))
            elements.append(
                Paragraph("Source Documents", self.styles["CategoryHeader"])
            )
...
class AnalyticsExcelExporter:
    ...
    def export(self, analytics_data: dict[str, Any]) -> bytes:
        ...
        self._create_classification_sheet(
            wb, analytics_data.get("classification_breakdown", {})
        )
    ...
    def _create_classification_sheet(self, wb: Workbook, data: dict):
        ...
        for idx, (label, value) in enumerate(
            zip(labels, values, strict=False), start=2
        ):
            ws[f"A{idx}"] = label
            ws[f"B{idx}"] = value
    ...
    def _create_compliance_sheet(self, wb: Workbook, data: dict):
        ...
        for idx, (date, pct) in enumerate(
            zip(dates, percentages, strict=False), start=2
        ):
            ws[f"A{idx}"] = date
            ws[f"B{idx}"] = pct
    ...
    def _create_coverage_sheet(self, wb: Workbook, data: dict):
        ...
        for row_idx, (row_label, row_data) in enumerate(
            zip(row_labels, matrix, strict=False), start=2
        ):
