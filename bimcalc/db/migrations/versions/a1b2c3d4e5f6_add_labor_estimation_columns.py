...
def upgrade() -> None:
    ...
    op.add_column('price_items', sa.Column('labor_hours', sa.Numeric(precision=10, scale=2), nullable=True))
    op.add_column('price_items', sa.Column('labor_code', sa.Text(), nullable=True))
    op.create_index(op.f('ix_price_items_labor_code'), 'price_items', ['labor_code'], unique=False)

    # Add source and confidence_score to item_mapping
    op.add_column('item_mapping', sa.Column('source', sa.Text(), nullable=True))
    op.add_column('item_mapping', sa.Column('confidence_score', sa.Numeric(precision=5, scale=2), nullable=True))
