...
async def run_migration(session: AsyncSession, dry_run: bool = False) -> None:
    ...
    console.print("[bold]Executing SQLite migration to add org_id to price_items...[/bold]")
    ...
    try:
        ...
        result = await session.execute(
            text("PRAGMA table_info(price_items)")
        )
        ...
        result = await session.execute(
            text("SELECT org_id, COUNT(*) FROM price_items WHERE is_current = 1 GROUP BY org_id")
        )
...
async def run_rollback(session: AsyncSession, dry_run: bool = False) -> None:
    ...
    if dry_run:
        console.print("[yellow]DRY RUN MODE - No changes will be made[/yellow]\n")
        console.print("[bold red]Rollback SQL (LIMITED - BACKUP RESTORE RECOMMENDED):[/bold red]")
    ...
    confirm = typer.confirm("This will only restore indexes, not remove org_id column. Continue?")
    ...
    try:
        ...
        await session.commit()
        console.print("[bold yellow]⚠[/bold yellow] Partial rollback completed (indexes only)")
...
def migrate(
    ...
    rollback: bool = typer.Option(False, "--rollback", help="Limited rollback (index changes only)"),
):
