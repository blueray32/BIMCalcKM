...
class AnalyticsEngine:
    ...
    async def get_item_price_history(self, item_code: str, org_id: str) -> Dict[str, Any]:
    ...
    async def compare_vendors(self, item_codes: List[str], org_id: str) -> Dict[str, Any]:
        ...
        for row in rows:
            comparison.append({
                ...
                "coverage_percent": (row.item_count / len(item_codes)) * 100 if item_codes else 0
            })
            
        return {"comparison": comparison}

    async def forecast_cost_trends(self, project_id: UUID, days: int = 90) -> Dict[str, Any]:
