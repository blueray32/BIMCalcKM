...
from bimcalc.reporting.dashboard_metrics import DashboardMetrics, compute_dashboard_metrics
...
async def generate_project_pdf_report(
    ...
    story.append(Paragraph(f"Project Cost Report", title_style))
    story.append(Paragraph(f"{project.display_name or project.project_id}", subtitle_style))
    ...
    data = [
        ...
        ["Matched Items", f"{metrics.matched_items} / {metrics.total_items} ({metrics.match_percentage:.1f}%)"],
    ...
    cat_result = await session.execute(category_query, {"org_id": org_id, "project_id": project_id})
