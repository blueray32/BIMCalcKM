...
async def verify_recommendations():
    ...
    async with get_session() as session:
        ...
        item_no_class = ItemModel(
            id=uuid4(), org_id=org_id, project_id=project_str,
            classification_code=None, quantity=Decimal("10.0"),
            family="Unclassified Item", type_name="Type A"
        )
        ...
        item_low_conf = ItemModel(
            id=uuid4(), org_id=org_id, project_id=project_str,
            classification_code="123", quantity=Decimal("10.0"),
            family="Low Conf Item", type_name="Type B"
        )
        session.add(item_low_conf)
        
        match_low_conf = MatchResultModel(
            id=uuid4(), item_id=item_low_conf.id, price_item_id=None,
            confidence_score=60.0, decision="manual-review",
            source="fuzzy_match", reason="weak", created_by="system"
        )
        ...
        item_high_cost = ItemModel(
            id=uuid4(), org_id=org_id, project_id=project_str,
            classification_code="123", quantity=Decimal("1.0"),
            family="Expensive Item", type_name="Type C"
        )
        session.add(item_high_cost)
        
        price_high = PriceItemModel(
            id=uuid4(), org_id=org_id, item_code="P_HIGH", region="EU",
            unit_price=Decimal("1000.00"), currency="EUR",
            source_name="Vendor A", description="Expensive", unit="ea", sku="SKU1",
            classification_code="123", source_currency="EUR"
        )
        session.add(price_high)
        
        match_high_cost = MatchResultModel(
            id=uuid4(), item_id=item_high_cost.id, price_item_id=price_high.id,
            confidence_score=95.0, decision="auto-accepted",
            source="fuzzy_match", reason="good", created_by="system"
        )
        ...
        try:
            ...
            response = client.get(f"/api/projects/{project_str}/intelligence/recommendations")
