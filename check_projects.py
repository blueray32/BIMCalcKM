...
async def check_projects():
    async with get_session() as session:
        ...
        for p in projects:
            print(f"Project: {p.display_name} (ID: {p.project_id}, Org: {p.org_id}, Created: {p.created_at})")

        # Test the specific query I used
        stmt_limit = select(ProjectModel).order_by(ProjectModel.created_at.desc()).limit(1)
