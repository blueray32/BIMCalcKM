...
from bimcalc.db.models import PriceItemModel, ItemModel, MatchResultModel
...
async def test_labor_estimation_flow(db_session):
    ...
    await session.execute(text("DELETE FROM price_items WHERE vendor_id = 'TEST_LABOR_INTEGRATION'"))
    await session.execute(text("DELETE FROM items WHERE org_id = 'test_org_integration'"))
    await session.execute(text("DELETE FROM match_results WHERE created_by = 'test_script_integration'"))
    await session.execute(text("DELETE FROM item_mapping WHERE created_by = 'test_script_integration'"))
...
"""
    ...
    try:
        ...
        stmt = text("SELECT labor_hours FROM price_items WHERE sku = 'LAB-INT-001' AND vendor_id = 'TEST_LABOR_INTEGRATION'")
        ...
        # Get Price Item
        stmt = text("SELECT id FROM price_items WHERE sku = 'LAB-INT-001' AND vendor_id = 'TEST_LABOR_INTEGRATION'")
        ...
        metrics = await compute_dashboard_metrics(session, "test_org_integration", "test_proj_integration")
